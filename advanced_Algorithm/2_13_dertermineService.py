...
headers = ['node_id','vehicle_serial','gps_time','latitude', 'longitude', 'altitude', 'speed', 'heading', 'POI', 'service', 'service_start_stop', 'passengers']
...
def match_Service(filepath, all_poi_normal, all_poi_normal_num):
    ...
    with open(filepath, 'r') as ss_v:  #with打开不用在意close
        ...
        for row in r_ss:
            ...
            ## 针对中途丢失数据的
            if row[8] not in miss_mid_passby_poi:
                miss_mid_passby_poi.append(row[8])
                if miss_mid_time_state:
            ...
            for i in matching_service:
                ...
                if pointer_POI[i] > 0 and check:
                    ...
                    if (mid_appear_counter // len(matching_service) >= 35) or \
                        ...
                        ### 针对中间丢失数据的情况,用final_service_list里面的上一个站点，也就是当前记录的最后一个来计算概率(>0.65)
                        if len(final_service_list) != 0:
                            for miss_mid_poi_item in miss_mid_passby_poi:
                                miss_mid_counter = 0
                                if miss_mid_poi_item in all_poi_normal[final_service_list[-1]]:
                                    miss_mid_counter += 1
                                else:
                                    continue
                            miss_mid_prob[final_service_list[-1]] = miss_mid_counter / all_poi_normal_num[final_service_list[-1]]                        
                            if miss_mid_prob[final_service_list[-1]] >= float(0.5):
                                print('0.5555555')
                                pointer_POI[final_service_list[-1]] = all_poi_normal_num[final_service_list[-1]]
                                final_time.append(miss_mid_start_time)
                                start_row.append(miss_mid_start_row)  #让后面计数的时候减100到0
                                stop_row.append(counter)
                        ...
                        if len(miss_headmost_passby_poi) != 0 and miss_headmost_state:
                            ...
                            if miss_headmost_state and miss_headmost_prob_self[miss_headmost_predict] >= float(0.6):                                
                                pointer_POI[miss_headmost_predict] = all_poi_normal_num[miss_headmost_predict]
                                start_service_time[miss_headmost_predict] = 'Pred-' + first_row_time
                                start_row.append(100)  #让后面计数的时候减100到0
                                stop_row.append(counter)
                            miss_headmost_passby_poi = []                        
