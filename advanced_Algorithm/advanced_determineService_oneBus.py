...
def getStations_Inorder():
    ...
    return(all_poi_normal)  #{'A1':[XX, XX, XX, XX], 'B2':[XX, XX, XX, XX]...}
    # print('------')
...
def match_Service(filepath, all_poi_normal, all_poi_normal_num):
    ...
    # 注意：每天的第一趟服务的所有站点都会被记录，因为假设是缺失的，但是只有在第一趟服务被判断成pause的时候才计算该概率
        ...
        for row in r_ss:
            ...
            if row[8] not in miss_mid_passby_poi and len(final_service_list) != 0:
                ...
                try:
                    ...
                    print(e)
                    print('Opps. But it does not matter! Go on!')
            ...
            for i in matching_service:
                ...
                try:
                    for miss_cont in range(len(miss_mid_poi)):                        
                        if all_poi_normal[i][pointer_POI[i]-1] in miss_mid_poi[miss_cont][0]:
                            if all_poi_normal[i][pointer_POI[i]+1] in miss_mid_poi[miss_cont][2]:
                ...
                # if (1 in [values for key, values in pointer_POI.items()]) and check:
                    ...
                    if (mid_appear_counter // len(matching_service) >= 35) or \
                        ...
                        ### 在遇到pause的情况下-针对中间丢失数据的情况,用final_service_list里面的上一个站点，也就是当前记录的最后一个来计算概率(>0.65)
                        ## 60% 匹配率即可通过
                        if len(final_service_list) != 0:
                            ...
                            for miss_mid_poi_item in miss_mid_passby_poi:
                                try:
                                    if miss_mid_poi_item in all_poi_normal[miss_mid_predict]:
                                        miss_mid_counter += 1
                                    else:
                                        continue
                            ...
                            miss_mid_counter = 0
                            if miss_mid_prob[miss_mid_predict] >= float(0.6):
    ...
    vehicle_id = filepath.split('/')[2].split('_')[0]
    ...
    with open(read_filepath, 'r') as f:
        ...
        with open(write_filepath, 'w', newline ='') as fnew:    #use 'w' for writing str and newline='' for deleting extra idle row
            ...
            for row in read:
                ...
                counter_all[row[1]] += 1
                for count in range(len(all_record[row[1]])):
                    if counter_all[row[1]] >= all_record[row[1]][count][2] and counter_all[row[1]] <= all_record[row[1]][count][3]:
                        write_state = 1                        
                        write_index = count
...
if __name__ == "__main__":
    ...
    # can be used to test a particular day
    match_Service('../oneDay/fenced_vehicle/2018-01-15/2052-2018-01-15.csv', all_poi_normal, all_poi_normal_num)
    ...
    #     result_path = '/'.join(['..', 'Final_Result', 'test-dfrow-' + filePath.split('_')[2]])
    ...
    # read_filepath = '../oneDay/fenced2017-09-04.csv'
    # write_filepath = 'final_2017-0904.csv'
