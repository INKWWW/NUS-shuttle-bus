        ...
        for row in r_ss:
            ...
            if row[8] == 'Ventus (Opp LT13)' and A2E_match_state:
                ...
                if A2E_match_counter > 12:                    
                    time_hour = int((row[2].split('T')[1].split(':')[0]).strip())
            ...
            for i in matching_service:
                ...
                if pointer_POI[i] == 1 and int(row[6]) == 0 and start_service_time_state:
                    start_service_time[i] = row[2]
                    # print("!!!!!start !!!!!!!", start_service_time)
                    if all_poi_normal[i][0] != row[8]:
                        start_service_time_state = False
                        print("!!!!!start !!!!!!!", start_service_time)
