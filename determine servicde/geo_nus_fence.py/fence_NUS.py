...
with open('../Veniam_BusLocation/2017_week43/2017-10-23.csv', 'r') as f_vehicle:  #with打开不用在意close
    ...
    # print(headings_v)              
    Vehicle = namedtuple('Vehicle', headings_v)   #用namedtuple 方便后面用headings来读取数据
    newfilename = '_'.join(['fenced_vehicle/', vehicleid, 'inside-2017-10-23.csv'])  #根据不同的车辆号码输入创建不同的文件名
    with open(newfilename, 'w', newline ='') as fnew:    #use 'w' for writing str and newline='' for deleting extra idle row
        ...
        for row_v in r_vehicle:
            ...
            if vehicle.node_id == vehicleid:
                ...
                in_or_out = myfunction.point_in_poly(float(vehicle.latitude), float(vehicle.longitude), polygon)
                print(in_or_out)
                if in_or_out is 'OUT':
                    continue
                else :  
