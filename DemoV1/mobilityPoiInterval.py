...
def writeIntervalInfo(read_filepath, write_filepath, POI_services_interval, POI_services_start):
    ...
    with open(read_filepath, 'r') as f:
        ...
        with open(write_filepath, 'w', newline ='') as fnew:    #use 'w' for writing str and newline='' for deleting extra idle row
            ...
            for row in r_f:
                if len(row) == 11:
                    while(service_index < len(POI_services_start[row[8]])):
                        service_start_time = POI_services_start[row[8]][service_index]  # service_start_time = {A1:[S1, S2, S3]}
                        service = list(service_start_time.keys())[0]
                        # print(service)
                        service_index += 1                
                        if row[9] == service:
                            date = row[2].split('T')[0]
                            time = row[2].split('T')[1].split('.')[0]
                            if time in service_start_time[service]:
                                start_time_index = service_start_time[service].index(time)                                
                                w_fnew.writerow([row[0], row[1], date, row[2], row[8], row[9], POI_services_interval[row[8]][service_index-1][service][start_time_index]])              
                                service_index = 0
                                break
                            
                            # # For every service belonging to a poi, 'index_poi_service' should be set to zero firstly
                            # index_poi_service = 0  # this index is to record the index of intervals in a particular poi's service

                            # if row[8] == poi:
                            #     # 进入站点
                            #     if row[9] == service and start_time_state:
                            #         date = row[2].split('T')[0]
                            #         w_fnew.writerow(row[0], row[1], date, row[2], row[8], row[9], all_intervals_one_poi[service][index_poi_service])
                            #         index_poi_service += 1
                            #         start_time_state = False
                            #     continue
...
if __name__ == '__main__':
    ...
    for poi in all_POI:
        ...
        for service in POI_services[poi]:
            ...
            start_list = {}
            start_list[service] = []
            service_ss = recordStartStop(read_filepath, poi, service)
    ...
    print('writing...')
    writeIntervalInfo(read_filepath, write_filepath, final_POI_services_interval, POI_services_start)
