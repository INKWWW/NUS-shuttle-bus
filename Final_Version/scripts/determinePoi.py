...
#################################################Geo Fencing############################################
'''Clean up data and rearrange data
...
def GeoFence_NusPoly(filepath, poi, polygon_NUS):
    ...
    with open(filepath, 'r') as f_vehicle:  #with打开不用在意close
        ...
        # print(headings_v)              
        Vehicle = namedtuple('Vehicle', headings_v)   #用namedtuple 方便后面用headings来读取数据
        newpath = '/'.join(['fenced', 'fenced-' + filepath.split('/')[-1]])
        ...
        with open(newpath, 'w', newline ='') as fnew:    #use 'w' for writing str and newline='' for deleting extra idle row
            ...
            for row_v in r_vehicle:
                ...
                if in_or_out is 'OUT':
                    ...
                    new_time = time.split('T')[0].split('-')[0] + '-' + time.split('T')[0].split('-')[1] + \
                               '-' + str(date_time) + 'T' + str(hour_time) + ':' + minsec_time   
...
def run(filepath):
    ...
    '''
    poi = ListOfPoi('../source/New List of NUS Shuttle Bus POIs.xlsx')
    polygon_NUS = NusPolygon('../source/NUS_polygon.xlsx')
    GeoFence_NusPoly(filepath, poi, polygon_NUS)
    print('Success!')
