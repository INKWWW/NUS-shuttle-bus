...
def handleEachFile(filepath):
    ...
    Arguments:
        filepath {[String]} -- [raw file path]
    '''
    # First step
    determinePoi.run(filepath)
    print('First step successfully')

    # Second step
    filepath2 = 'fenced/fenced-' + filepath.split('/')[-1]
    seperate_vehicle.run(filepath2)
    print('Second step successfully')

    # Third step
    folderpath = 'fenced_vehicle/' + filepath.split('/')[-1].split('.')[0]
    read_filepath = filepath2
    write_filepath = '../output/' + filepath.split('.')[0] + '/' + 'final_' + filepath.split('/')[-1]
...
def aggregate(folderpath):
    ...
    for filepath in fileNameList[0]:
        ...
        filepath_new = folderpath + '/' + filepath
