...
headers = ['node_id','vehicle_serial','date', 'gps_time','latitude', 'longitude', 'altitude', 'speed', 'heading', 'POI', 'service', 'service_start_stop']  # 11
# service = ['A1', 'A2', 'B1', 'B2', 'C*', 'C', 'D1', 'D2', 'A1E', 'A2E', 'BTC1', 'BTC2', 'UT-FoS'] #13
service = [] # Get service list from the .xlsx file
...
def get_NUS_fence_vertex():
    wpoly = load_workbook('../source/NUS_polygon.xlsx')
...
def getStations_Inorder(service):
    ...
    all_POI = []
    j = 0
    for poi_col in range (1, poi_all_column):
        j += 1
        poi_normal = []
...
def match_Service(filepath, all_poi_normal, all_poi_normal_num, service):
        ...
        for row in r_ss:
            ...
            for i in matching_service:
                ...
                if pointer_POI[i] == all_poi_normal_num[i]:
                    ...
                    count_break = 0
                    while(int(row[6]) > 0) and (count_break < 100):
                        count_break += 1
                        # print(count_break)
                        try:
                            row = next(r_ss)
        ...
        with open(write_filepath, 'w', newline ='') as fnew:    #use 'w' for writing str and newline='' for deleting extra idle row
            ...
            for row in read:
                ...
                if write_state == 1 and write_start_state == 1:  # 标志服务的起始
                    writer.writerow([row[0], row[1], date, row[2], row[3], row[4], row[5], row[6], row[7], row[8], all_record[row[0]][write_start][0], 'service_start'])
                    # print('start-- ', write_start, counter_all[row[0]])
                if write_state == 1 and write_stop_state == 1:  # 标志服务的停止
                    writer.writerow([row[0], row[1], date, row[2], row[3], row[4], row[5], row[6], row[7], row[8], all_record[row[0]][write_stop][0], 'service_stop'])
                    # print('stop-- ', write_stop, counter_all[row[0]])
                if write_state == 1 and write_start_state != 1 and write_stop_state != 1:
                    writer.writerow([row[0], row[1], date, row[2], row[3], row[4], row[5], row[6], row[7], row[8], all_record[row[0]][write_index][0], 'Running'])
                if write_state != 1:
                    writer.writerow([row[0], row[1], date, row[2], row[3], row[4], row[5], row[6], row[7], '', '', ''])
    print('Success!')
...
def run(folderpath, read_filepath, write_filepath):
    ...
    MyFunctions.checkPath(write_filepath)
    all_poi_normal_num = {}
    all_poi_normal = getStations_Inorder(service)
    for i in service:
    ...
    for filePath in visitAllFile(folderpath)[0]:
        ...
        tmp_df, service_time_pair = match_Service(dir_filePath, all_poi_normal, all_poi_normal_num, service)
        df_row = pd.concat([df_row, tmp_df], axis = 1)
        date = '-'.join(filePath.split('-')[1:])
        result_path = ''.join(['..output/schedule/', 'test-dfrow-', date])
